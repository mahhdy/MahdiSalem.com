...
/**
 * Script to add 'interface' field to all content frontmatter
 * Maps existing category names to proper category slugs from categories.ts
 */
...
function determineInterfaceCategory(frontmatter) {
  // If already has interface, validate it
  if (frontmatter.interface && VALID_CATEGORIES.includes(frontmatter.interface)) {
    return frontmatter.interface;
  }
...
function processFile(filePath) {
  try {
    ...
    const interfaceCategory = determineInterfaceCategory(parsed.data);

    // Add interface field if missing or invalid
    if (!parsed.data.interface || !VALID_CATEGORIES.includes(parsed.data.interface)) {
      parsed.data.interface = interfaceCategory;

      // Rebuild frontmatter with interface field
      const newContent = matter.stringify(parsed.content, parsed.data);
      fs.writeFileSync(filePath, newContent, 'utf8');

      console.log(`✓ Updated: ${path.basename(filePath)} -> interface: ${interfaceCategory}`);
      return true;
    } else {
      console.log(`  Skipped: ${path.basename(filePath)} (already has valid interface)`);
      return false;
    }
  } catch (error) {
...
// Main execution
console.log('Adding interface categories to content files...\n');
...
console.log(`\n✅ Complete! Updated ${totalUpdated} files with interface categories.`);
