  ...
  return (tree) => {
    visit(tree, 'code', (node, index, parent) => {
      if (node.lang === 'mermaid') {
        ...
        // ✅ NEW - safe auto-quoting
        processedValue = processedValue.replace(
          // Only match bracket content that is NOT already quoted
          /([$$\(\{>])(?!")([^$$\)\}<"]+?)([\]\)\}])/g,
          (match, open, text, close) => {
            // Only add quotes if:
            // 1. Contains Farsi characters
            // 2. Not already quoted
            // 3. Not a single word (likely a node ID like A, B1, etc.)
            // 4. Not inside a style definition
            if (
              /[\u0600-\u06FF]/.test(text) &&
              !text.startsWith('"') &&
              !text.endsWith('"') &&
