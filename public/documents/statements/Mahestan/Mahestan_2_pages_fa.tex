...
%% فونت
\setmainfont[Scale=1.1]{Vazirmatn}
\setdigitfont[Scale=1.1]{Vazirmatn}
\newfontfamily\persianfont[Scale=1.1]{Vazirmatn}
...
\begin{document}
	...
	%% ─── \begin{latin} REMOVED ───
		\begin{tikzpicture}[remember picture, overlay]
			...
			([xshift=1cm, yshift=-4.4cm]current page.north west) --
			([xshift=6cm, yshift=-4.4cm]current page.north west);
			
			%% باکس‌های مشکل و راه‌حل
			\node[rectangle, rounded corners=8pt, fill=LightRed, draw=PrimaryRed,
			text width=8.5cm, align=right, inner sep=10pt,
			font=\small]
			...
			%% ═══ بخش ۲: ترکیب مجلس ═══
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-8.5cm]current page.north west)
			{\rl{ترکیب مجلس مهستان}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-8.9cm]current page.north west) --
			([xshift=7cm, yshift=-8.9cm]current page.north west);
			
			%% نمودار دایره‌ای ترکیب
			\begin{scope}[shift={([xshift=5cm, yshift=-12.5cm]current page.north west)}]
				...
				\fill[PrimaryBlue] (0:2.2cm) arc (0:288:2.2cm) --
				(288:1.9cm) arc (288:0:1.9cm) -- cycle;
				\fill[GoldYellow] (288:2.2cm) arc (288:336:2.2cm) --
				(336:1.9cm) arc (336:288:1.9cm) -- cycle;
				\fill[PrimaryPurple] (336:2.2cm) arc (336:360:2.2cm) --
				(360:1.9cm) arc (360:336:1.9cm) -- cycle;
			\end{scope}
			...
			%% ═══ بخش ۳: ۶ اصل راهنما ═══
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-16.5cm]current page.north west)
			{\rl{۶ اصل راهنما}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-16.9cm]current page.north west) --
			([xshift=5cm, yshift=-16.9cm]current page.north west);
			...
			\foreach \i/\title/\col in {
				...
				at ([xshift={1.5cm+(\i-1)*6cm}, yshift=-18.5cm]current page.north west)
				{\rl{\i}};
				\node[anchor=north, font=\small\bfseries\color{\col}, text width=4.5cm, align=center]
				at ([xshift={1.5cm+(\i-1)*6cm}, yshift=-19.3cm]current page.north west)
				{\title};
			}
			...
			\foreach \i/\title/\col in {
				...
				\node[circle, fill=\col, minimum size=1.2cm,
				font=\large\bfseries\color{white}]
				at ([xshift={1.5cm+(\i-4)*6cm}, yshift=-21cm]current page.north west)
				{\rl{\i}};
				\node[anchor=north, font=\small\bfseries\color{\col}, text width=4.5cm, align=center]
				at ([xshift={1.5cm+(\i-4)*6cm}, yshift=-21.8cm]current page.north west)
				{\title};
			}
			
			%% ═══ بخش ۴: خط زمانی ═══
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-23.5cm]current page.north west)
			{\rl{خط زمانی گذار}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-23.9cm]current page.north west) --
			([xshift=5.5cm, yshift=-23.9cm]current page.north west);
			...
			\foreach \x/\phase/\col/\label in {
				...
				\fill[\col] ([xshift=\x cm, yshift=-25.5cm]current page.north west) circle (6pt);
				\node[anchor=south, font=\scriptsize\bfseries\color{\col}]
				at ([xshift=\x cm, yshift=-25.3cm]current page.north west) {\rl{فاز \phase}};
				\node[anchor=north, font=\tiny\color{DarkGray}]
				at ([xshift=\x cm, yshift=-25.7cm]current page.north west) {\label};
			}
	...
	%% صفحه‌ی دوم: عدالت انتقالی، اقدامات فوری، فراخوان
	%% ─── \begin{latin} REMOVED ───
		\begin{tikzpicture}[remember picture, overlay]
			...
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-3.5cm]current page.north west)
			{\rl{۵ ستون عدالت انتقالی}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-3.9cm]current page.north west) --
			([xshift=7cm, yshift=-3.9cm]current page.north west);
			...
			\foreach \i/\title/\col/\items in {
				...
				at ([xshift={1.8cm+(\i-1)*3.6cm}, yshift=-6.5cm]current page.north west)
				{%
					\textbf{\color{\col}\title}\\[4pt]
					\rl{\rule{2cm}{0.3pt}}\\[3pt]
					{\scriptsize\items}%
				};
			}
			...
			%% ═══ بخش ۲: ۱۰ اقدام فوری ═══
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-9.5cm]current page.north west)
			{\rl{۱۰ اقدام فوری — هفته‌ی اول}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-9.9cm]current page.north west) --
			([xshift=8cm, yshift=-9.9cm]current page.north west);
			...
			\foreach \i/\action in {
				...
				at ([xshift=1.5cm, yshift={-10.5cm-(\i-1)*0.9cm}]current page.north west) {\rl{\i}};
				\node[anchor=west, font=\small]
				at ([xshift=2.1cm, yshift={-10.5cm-(\i-1)*0.9cm}]current page.north west) {\action};
			}
			...
			\foreach \i/\action in {
				...
				\node[circle, fill=PrimaryGreen, minimum size=0.7cm,
				font=\small\bfseries\color{white}]
				at ([xshift=10.5cm, yshift={-10.5cm-(\i-6)*0.9cm}]current page.north west) {\rl{\i}};
				\node[anchor=west, font=\small]
				at ([xshift=11.1cm, yshift={-10.5cm-(\i-6)*0.9cm}]current page.north west) {\action};
			}
			
			%% ═══ بخش ۳: ۱۴ اصل غیرقابل‌نقض ═══
			\node[anchor=north west, font=\Large\bfseries\color{DeepBlue}]
			at ([xshift=1cm, yshift=-16cm]current page.north west)
			{\rl{۱۴ اصل غیرقابل‌نقض}};
			
			\draw[DeepBlue, line width=1.5pt]
			([xshift=1cm, yshift=-16.4cm]current page.north west) --
			([xshift=6cm, yshift=-16.4cm]current page.north west);
			...
			at ([xshift=10.25cm, yshift=-18.8cm]current page.north west)
			{%
				{\small%
			...
			at ([xshift=10.25cm, yshift=-23.5cm]current page.north west)
			{%
			...
			at ([yshift=-26cm]current page.north)
